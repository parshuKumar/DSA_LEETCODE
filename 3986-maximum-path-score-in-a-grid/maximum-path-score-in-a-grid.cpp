...
public:
    int maxPathScore(vector<vector<int>>& grid, int k) {
        ...
        vector<vector<vector<int>>> dp(grid.size() + 1, vector<vector<int>> (grid[0].size() + 1, vector<int>(k + 2, -1)));
    ...
    int solve(vector<vector<int>>& grid, int row, int col, int k, vector<vector<vector<int>>> &dp){
        ...
        if(dp[row][col][k + 1] != -1) return dp[row][col][k + 1];

        int down = solve(grid, row + 1, col, k, dp);
        int right = solve(grid, row, col + 1, k, dp);

        int best = max(down, right);
        return dp[row][col][k +1] = (best == -1e9 ? -1e9 : grid[row][col] + best); 
    }
